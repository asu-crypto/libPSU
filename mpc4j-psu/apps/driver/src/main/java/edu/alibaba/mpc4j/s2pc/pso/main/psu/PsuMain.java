...
public class PsuMain extends AbstractMainTwoPartyPto {
    ...
    /**
     * FHE-heavy PSU warmup: small server, moderate client (unbalanced direction).
     */
    private static final int FHE_WARMUP_SERVER_SET_SIZE = 1 << 6;
    private static final int FHE_WARMUP_CLIENT_SET_SIZE = 1 << 10;
    /**
     * element byte length
     */
    ...
    /**
     * skip warmup round (optional property {@code skip_warmup=true})
     */
    private final boolean skipWarmup;
    /**
     * skip explicit benchmark GC calls (optional property {@code skip_gc=true})
     */
    ...
    /**
     * Unbalanced workloads that use a light asymmetric warmup (64 x 1024) instead of 1024 x 1024.
     */
    private static boolean usesAsymmetricWarmup(PsuConfig config) {
        return false;
    }

    private static boolean isTwoSidedPsu(PsuConfig config) {
        return edu.alibaba.libpsu.factory.ProtocolRegistry.usesTwoSidedPublicFactory(config);
    }
    ...
    public void runParty1(Rpc serverRpc, Party clientParty) throws IOException, MpcAbortException {
        ...
        if (!skipWarmup) {
            ...
            if (usesAsymmetricWarmup(psuConfig)) {
                PsuBenchmarkUtils.generateBytesInputFiles(
                    FHE_WARMUP_SERVER_SET_SIZE, FHE_WARMUP_CLIENT_SET_SIZE, WARMUP_ELEMENT_BYTE_LENGTH
                );
            } else {
        ...
        for (int setSizeIndex = 0; setSizeIndex < setSizeNum; setSizeIndex++) {
            PsuBenchmarkUtils.generateBytesInputFiles(serverSetSizes[setSizeIndex], clientSetSizes[setSizeIndex], elementByteLength);
        }
        ...
        if (!skipWarmup) {
            if (usesAsymmetricWarmup(psuConfig)) {
                warmupServer(
                    serverRpc, clientParty, psuConfig, taskId,
                    FHE_WARMUP_SERVER_SET_SIZE, FHE_WARMUP_CLIENT_SET_SIZE
                );
            } else {
                warmupServer(serverRpc, clientParty, psuConfig, taskId, WARMUP_SET_SIZE, WARMUP_SET_SIZE);
            }
            runBenchmarkGc();
            taskId++;
        }
    ...
    private Set<ByteBuffer> readServerElementSet(int setSize, int elementByteLength) throws IOException {
        ...
        InputStreamReader inputStreamReader = new InputStreamReader(
            Files.newInputStream(Paths.get(PsuBenchmarkUtils.getBytesFileName(PsuBenchmarkUtils.BYTES_SERVER_PREFIX, setSize, elementByteLength))),
    ...
    private void warmupServer(Rpc serverRpc, Party clientParty, PsuConfig config, int taskId,
        ...
        if (isTwoSidedPsu(config)) {
            ...
            psuServer.psu(serverElementSet, clientSetSize, WARMUP_ELEMENT_BYTE_LENGTH);
            psuServer.getRpc().synchronize();
    ...
    private void runTwoSidedServer(Rpc serverRpc, Party clientParty, PsuConfig config, int taskId,
                                   Set<ByteBuffer> serverElementSet, int clientSetSize, int elementByteLength,
                                   PrintWriter printWriter) throws MpcAbortException {
        int serverSetSize = serverElementSet.size();
        PsuTwoSidedServer psuServer = PsuFactory.createTwoSidedServer(serverRpc, clientParty, config);
    ...
    public void runParty2(Rpc clientRpc, Party serverParty) throws IOException, MpcAbortException {
        ...
        if (!skipWarmup) {
            if (usesAsymmetricWarmup(psuConfig)) {
                warmupClient(
                    clientRpc, serverParty, psuConfig, taskId,
                    FHE_WARMUP_SERVER_SET_SIZE, FHE_WARMUP_CLIENT_SET_SIZE
                );
            } else {
                warmupClient(clientRpc, serverParty, psuConfig, taskId, WARMUP_SET_SIZE, WARMUP_SET_SIZE);
            }
            runBenchmarkGc();
            taskId++;
        }
    ...
    private Set<ByteBuffer> readClientElementSet(int setSize, int elementByteLength) throws IOException {
        ...
        InputStreamReader inputStreamReader = new InputStreamReader(
            Files.newInputStream(Paths.get(PsuBenchmarkUtils.getBytesFileName(PsuBenchmarkUtils.BYTES_CLIENT_PREFIX, setSize, elementByteLength))),
            CommonConstants.DEFAULT_CHARSET
        );
        BufferedReader bufferedReader = new BufferedReader(inputStreamReader);
    ...
    private void warmupClient(Rpc clientRpc, Party serverParty, PsuConfig config, int taskId,
                              int serverSetSize, int clientSetSize) throws IOException, MpcAbortException {
        ...
        if (isTwoSidedPsu(config)) {
            ...
            psuClient.psu(clientElementSet, serverSetSize, WARMUP_ELEMENT_BYTE_LENGTH);
            psuClient.getRpc().synchronize();
    ...
    private void runTwoSidedClient(Rpc clientRpc, Party serverParty, PsuConfig config, int taskId,
                                   Set<ByteBuffer> clientElementSet, int serverSetSize, int elementByteLength,
                                   PrintWriter printWriter) throws MpcAbortException {
        int clientSetSize = clientElementSet.size();
        PsuTwoSidedClient psuClient = PsuFactory.createTwoSidedClient(clientRpc, serverParty, config);
