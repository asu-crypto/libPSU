...
public class UpsuMain extends AbstractMainTwoPartyPto {
    ...
    /**
     * Legacy PSU key reused by fair-bench configs (pre split).
     */
    private static final String LEGACY_PSU_PTO_NAME_KEY = "psu_pto_name";
    /**
    ...
    public UpsuMain(Properties properties, String ownName) {
        ...
        // For benchmark compatibility: allow using legacy psu_pto_name = TCL23
        if (properties.containsKey(PTO_NAME_KEY)) {
            ...
            upsuMainType = MainPtoConfigUtils.readEnum(UpsuMainType.class, properties, LEGACY_PSU_PTO_NAME_KEY);
        } else {
    ...
    public void runParty1(Rpc serverRpc, Party clientParty) throws IOException, MpcAbortException {
        ...
        PsuBenchmarkUtils.generateBytesInputFiles(WARMUP_SERVER_SET_SIZE, WARMUP_CLIENT_SET_SIZE, WARMUP_ELEMENT_BYTE_LENGTH);
    ...
    private Set<ByteBuffer> readServerElementSet(int setSize, int elementByteLength) throws IOException {
        ...
        InputStreamReader inputStreamReader = new InputStreamReader(
            new FileInputStream(PsuBenchmarkUtils.getBytesFileName(PsuBenchmarkUtils.BYTES_SERVER_PREFIX, setSize, elementByteLength)),
    ...
    private Set<ByteBuffer> readClientElementSet(int setSize, int elementByteLength) throws IOException {
        ...
        InputStreamReader inputStreamReader = new InputStreamReader(
            new FileInputStream(PsuBenchmarkUtils.getBytesFileName(PsuBenchmarkUtils.BYTES_CLIENT_PREFIX, setSize, elementByteLength)),
            CommonConstants.DEFAULT_CHARSET
        );
        BufferedReader bufferedReader = new BufferedReader(inputStreamReader);
