...
/**
 * HN12 PSI client (P1): set X, learns X ∩ Y (Protocol 5, π∩).
 */
public class Hn12PsiClient extends AbstractPsiClient implements PsiClient {
    ...
    public Hn12PsiClient(Rpc clientRpc, Party serverParty, Hn12PsiConfig config) {
        super(Hn12PsiPtoDesc.getInstance(), clientRpc, serverParty, config);
        this.config = config;
    }
    ...
    public PsiClientOutput psi(Set<ByteBuffer> clientElementSet, int serverElementSize, int elementByteLength)
        ...
        Hn12MessageIO.sendStep(rpc, encodeTaskId, getPtoDesc(), extraInfo, ownParty(), otherParty(), PtoStep.P1_SEND_KEYS.ordinal(),
            List.of(Hn12WireCodec.encodeKeys(group.serializeParameters(), h, hPrime, dlH, dlHp, group)));

        // Step 2: balanced allocation
        ...
        Hn12MessageIO.sendStep(rpc, encodeTaskId, getPtoDesc(), extraInfo, ownParty(), otherParty(), PtoStep.P1_SEND_ALLOC.ordinal(),
        ...
        Hn12MessageIO.sendStep(rpc, encodeTaskId, getPtoDesc(), extraInfo, ownParty(), otherParty(), PtoStep.P1_SEND_ENC_POLYS.ordinal(),
        ...
        Hn12MessageIO.sendStep(rpc, encodeTaskId, getPtoDesc(), extraInfo, ownParty(), otherParty(), PtoStep.P1_SEND_POLY_PROOF.ordinal(),
            List.of(Hn12WireCodec.encodePolyProof(polyProof)));

        // Step 6–7: recover intersection
        ...
        for (int alpha = 0; alpha < serverElementSize; alpha++) {
            ...
            if (!dec0.equals(group.identity()) && !dec1.equals(group.identity())) {
                continue;
            }
            ...
            for (ByteBuffer clientElement : clientElementArrayList) {
                if (Hn12ElementCodec.encodeToScalar(clientElement, group.getQ(), domain).equals(y)) {
                    intersection.add(clientElement.duplicate());
                    break;
                }
            }
        }
