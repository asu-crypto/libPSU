...
import edu.alibaba.mpc4j.s2pc.pso.psu.ks05.Ks05PsuPtoDesc.PtoStep;
...
/**
 * C_KisSon05 polynomial-based PSU client (output party).
 */
public class Ks05PsuClient extends AbstractPsuClient implements PsuClient {
    private final Ks05PsuConfig config;
    private final PheEngine pheEngine;

    private PhePrivateKey sk;
    private PhePublicKey pk;

    public Ks05PsuClient(Rpc clientRpc, Party serverParty, Ks05PsuConfig config) {
        super(Ks05PsuPtoDesc.getInstance(), clientRpc, serverParty, config);
        this.config = config;
    ...
    public void init(int maxClientElementSize, int maxServerElementSize) throws MpcAbortException {
        ...
        Ks05PsuUtils.checkSetSizeLimit(maxServerElementSize, maxClientElementSize, config.getMaxSetSize());
    ...
    public PsuClientOutput psu(Set<ByteBuffer> clientElementSet, int serverElementSize, int elementByteLength)
        ...
        Ks05PsuUtils.checkSetSizeLimit(serverElementSize, clientElementSize, config.getMaxSetSize());
        ...
        for (ByteBuffer e : clientElementArrayList) {
            BigInteger root = Ks05PsuUtils.elementToInteger(Ks05PsuUtils.elementBytes(e));
            MpcAbortPreconditions.checkArgument(
                root.compareTo(modulus) < 0, "client element is outside the C_KisSon05 plaintext ring"
            );
            roots.add(root);
        }
        List<BigInteger> coeffs = Ks05PsuUtils.buildRootPolynomial(roots, modulus);
        ...
        for (int j = 0; j < serverElementSize; j++) {
            ...
            MpcAbortPreconditions.checkArgument(y.signum() > 0, "invalid nonzero C_KisSon05 tuple");
            MpcAbortPreconditions.checkArgument(
                y.gcd(modulus).equals(BigInteger.ONE), "non-invertible C_KisSon05 tuple denominator"
            );
            BigInteger s = Ks05PsuUtils.recoverServerElement(x, y, modulus);
            union.add(ByteBuffer.wrap(Ks05PsuUtils.decodeElement(s, elementByteLength)));
        }
        stopWatch.stop();
