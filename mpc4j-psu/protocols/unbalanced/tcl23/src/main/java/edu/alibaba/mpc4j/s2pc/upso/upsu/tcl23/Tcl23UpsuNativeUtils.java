...
public class Tcl23UpsuNativeUtils {
    ...
    /**
     * Paterson–Stockmeyer evaluation with optional FHE public key (needed for sparse low-degree remainders in DGG25 P2).
     * Pass {@code null} for {@code publicKey} to use relin-key-only evaluation.
     */
    public static native byte[] evalPs(byte[] encryptionParams, byte[] publicKey, byte[] relinKeys,
                                       List<byte[]> plaintexts, List<byte[]> ciphertexts, int psLowDegree,
                                       long[] mask);
     ...
     * @return encrypted matches.
     */
    public static native byte[] naiveComputeMatches(byte[] encryptionParams, List<byte[]> plaintexts,
     ...
     */
    public static native long[] decodeReply(byte[] encryptionParameters, byte[] secretKey, byte[] encryptedResponse);
}
