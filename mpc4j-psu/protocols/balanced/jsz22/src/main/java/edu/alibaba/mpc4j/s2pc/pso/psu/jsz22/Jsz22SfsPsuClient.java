...
public class Jsz22SfsPsuClient extends AbstractOoPsuClient {
    ...
    public PsuClientOutput psu(Set<ByteBuffer> clientElementSet, int serverElementSize, int elementByteLength)
        ...
        aPrimeArray = new byte[binNum][];
        for (int binIndex = 0; binIndex < binNum; binIndex++) {
            aPrimeArray[binIndex] = firstOsnReceiverOutput.getShare(binIndex);
        }
        stopWatch.stop();
        ...
        aPrimeArray = null;
        byte[][] s2Array = new byte[binNum][];
        for (int binIndex = 0; binIndex < binNum; binIndex++) {
            s2Array[binIndex] = secondOsnSenderOutput.getShare(binIndex);
        }
        stopWatch.stop();
        ...
        byte[][] zsArray = new byte[zsPayload.size()][];
    ...
    private void handleCuckooHashKeyPayload(List<byte[]> cuckooHashKeyPayload) throws MpcAbortException {
        ...
        hashBin = new EmptyPadHashBin<ByteBuffer>(envType, binNum, clientElementSize, cuckooHashKeys);
        hashBin.insertItems(clientElementArrayList);
    }

    private List<byte[]> generateClientOprfPayload() {
        ArrayList<byte[]> clientOprfPayload = new ArrayList<byte[]>(binNum * maxBinSize);
        for (int binIndex = 0; binIndex < binNum; binIndex++) {
            // For each y_j ∈ Y_B[π(i)], R adds F(k, y_j ⊕ a′_i) to I_i
            java.util.LinkedHashSet<ByteBuffer> distinct = new java.util.LinkedHashSet<ByteBuffer>();
            for (HashBinEntry<ByteBuffer> entry : hashBin.getBin(firstPi[binIndex])) {
                distinct.add(entry.getItem());
            }
            ArrayList<ByteBuffer> bin = new ArrayList<ByteBuffer>(distinct);
            ...
            for (int index = 0; index < oprfs.length; index++) {
                clientOprfPayload.add(oprfs[index]);
            }
        }
        hashBin = null;
        firstPi = null;
