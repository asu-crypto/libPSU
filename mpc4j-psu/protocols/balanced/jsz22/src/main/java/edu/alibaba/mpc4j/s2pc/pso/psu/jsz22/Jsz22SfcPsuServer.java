...
public class Jsz22SfcPsuServer extends AbstractOoPsuServer {
    ...
    public void psu(Set<ByteBuffer> serverElementSet, int clientElementSize, int elementByteLength)
        ...
        for (int binIndex = 0; binIndex < binNum; binIndex++) {
            aPrimeArray[binIndex] = osnReceiverOutput.getShare(binIndex);
        }
        stopWatch.stop();
    ...
    private void handleCuckooHashKeyPayload(List<byte[]> cuckooHashKeyPayload) throws MpcAbortException {
        ...
        hashes = new Prf[cuckooHashKeyPayload.size()];
        for (int i = 0; i < cuckooHashKeyPayload.size(); i++) {
            Prf prf = PrfFactory.createInstance(envType, Integer.BYTES);
            prf.setKey(cuckooHashKeyPayload.get(i));
            hashes[i] = prf;
        }
    }

    private List<byte[]> generateServerOprfPayload() {
        List<byte[]> serverOprfPayload = new ArrayList<byte[]>(serverElementSize * cuckooHashNum);
        for (ByteBuffer element : serverElementArrayList) {
            java.util.LinkedHashSet<Integer> positionSet = new java.util.LinkedHashSet<Integer>();
            for (Prf hash : hashes) {
                positionSet.add(hash.getInteger(element.array(), binNum));
            }
            int[] positions = new int[positionSet.size()];
            int posIdx = 0;
