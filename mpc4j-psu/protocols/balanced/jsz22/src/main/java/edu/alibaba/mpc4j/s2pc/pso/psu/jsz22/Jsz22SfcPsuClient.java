...
public class Jsz22SfcPsuClient extends AbstractOoPsuClient {
    ...
    public PsuClientOutput psu(Set<ByteBuffer> clientElementSet, int serverElementSize, int elementByteLength)
        ...
        byte[][] yVector = new byte[binNum][];
        for (int binIndex = 0; binIndex < binNum; binIndex++) {
            yVector[binIndex] = cuckooHashBin.getHashBinEntry(binIndex).getItemByteArray();
        }
        cuckooHashBin = null;
        ...
        byte[][] aArray = new byte[binNum][];
        for (int i = 0; i < binNum; i++) {
            aArray[i] = osnSenderOutput.getShare(i);
        }
        stopWatch.stop();
        ...
        clientOprfSet = new HashSet<ByteBuffer>(binNum);
        for (int i = 0; i < binNum; i++) {
            clientOprfSet.add(ByteBuffer.wrap(oprfOutputMap.digestToBytes(oprfReceiverOutput.getPrf(i))));
        }
        stopWatch.stop();
        ...
        Set<ByteBuffer> union = new HashSet<ByteBuffer>(serverElementSize);
    ...
    private boolean[] handleServerOprfPayload(List<byte[]> serverOprfPayload) throws MpcAbortException {
        ...
        byte[][] serverFlattenOprfs = new byte[serverOprfPayload.size()][];
        for (int i = 0; i < serverOprfPayload.size(); i++) {
            serverFlattenOprfs[i] = serverOprfPayload.get(i);
        }
        for (int index = 0; index < serverElementSize; index++) {
