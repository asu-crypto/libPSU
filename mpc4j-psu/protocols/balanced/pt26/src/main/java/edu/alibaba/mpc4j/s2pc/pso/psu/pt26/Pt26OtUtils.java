...
public final class Pt26OtUtils {
    ...
    public static List<byte[]> receiverDecrypt(
        ...
        Prg prg = PrgFactory.createInstance(envType, messageByteLength);
        ArrayList<byte[]> messages = new ArrayList<>(n);
        for (int index = 0; index < n; index++) {
    ...
    public static List<byte[]> runSenderOneOfThree(
        CoreCotSender coreCotSender, List<byte[]> w0, List<byte[]> w1, List<byte[]> w2,
        int messageByteLength, edu.alibaba.mpc4j.common.tool.EnvType envType, SecureRandom secureRandom
    ) throws MpcAbortException {
        Objects.requireNonNull(secureRandom, "secureRandom");
        int n = w0.size();
        List<byte[]> payload0 = runSenderOneOfTwo(
            coreCotSender, w0, randomMessages(n, messageByteLength, secureRandom), messageByteLength, envType
        );
        List<byte[]> payload1 = runSenderOneOfTwo(
            coreCotSender, w1, randomMessages(n, messageByteLength, secureRandom), messageByteLength, envType
        );
        List<byte[]> payload2 = runSenderOneOfTwo(
            coreCotSender, w2, randomMessages(n, messageByteLength, secureRandom), messageByteLength, envType
        );
        ArrayList<byte[]> merged = new ArrayList<>(n);
        for (int i = 0; i < n; i++) {
    ...
    public static List<byte[]> runReceiverOneOfThree(
        ...
        ArrayList<byte[]> out = new ArrayList<>(n);
        for (int i = 0; i < n; i++) {
            if (choices[i] == 0) {
                out.add(m0.get(i));
            } else if (choices[i] == 1) {
    ...
    private static List<byte[]> randomMessages(int n, int messageByteLength, SecureRandom secureRandom) {
