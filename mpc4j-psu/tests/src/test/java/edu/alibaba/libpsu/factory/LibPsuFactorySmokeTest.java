...
public class LibPsuFactorySmokeTest extends AbstractTwoPartyMemoryRpcPto {
    ...
    public void createSmallEcElligatorPsuParties() throws Exception {
        ...
        PsuServer psuServer = LibPsu.createServer(firstRpc, secondRpc.ownParty(), config);
        PsuClient psuClient = LibPsu.createClient(secondRpc, firstRpc.ownParty(), config);
    ...
    public void createKs05PsiParties() {
        ...
        p.setProperty("silent_cot", "true");
        var config = LibPsu.createPsiConfig(p);
        PsiServer server = LibPsu.createPsiServer(firstRpc, secondRpc.ownParty(), config);
        PsiClient client = LibPsu.createPsiClient(secondRpc, firstRpc.ownParty(), config);
    ...
    public void createKs05AndHn12PsuParties() {
        ...
        PsuConfig ksConfig = LibPsu.createPsuConfig(ks);
        Assert.assertEquals(PsuType.C_KisSon05, ksConfig.getPtoType());
        Assert.assertNotNull(LibPsu.createServer(firstRpc, secondRpc.ownParty(), ksConfig));
        Assert.assertNotNull(LibPsu.createClient(secondRpc, firstRpc.ownParty(), ksConfig));

        Properties hn = new Properties();
        hn.setProperty(PsuConfigUtils.PSU_PTO_NAME_KEY, "JOC:HazNis12");
        PsuConfig hnConfig = LibPsu.createPsuConfig(hn);
        Assert.assertEquals(PsuType.JOC_HazNis12, hnConfig.getPtoType());
        Assert.assertTrue(LibPsu.usesTwoSidedPublicFactory(hnConfig));
        Assert.assertNotNull(LibPsu.createTwoSidedServer(firstRpc, secondRpc.ownParty(), hnConfig));
        Assert.assertNotNull(LibPsu.createTwoSidedClient(secondRpc, firstRpc.ownParty(), hnConfig));
    }
}
