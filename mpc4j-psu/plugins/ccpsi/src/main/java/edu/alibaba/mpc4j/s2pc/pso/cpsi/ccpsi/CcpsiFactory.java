...
public class CcpsiFactory implements PtoFactory {
    ...
    public static <X> CcpsiServer<X> createServer(Rpc serverRpc, Party clientParty, CcpsiConfig config) {
        ...
        switch (type) {
            case PSTY19:
                return new Psty19CcpsiServer<X>(serverRpc, clientParty, (Psty19CcpsiConfig) config);
            case CGS22:
                return new Cgs22CcpsiServer<X>(serverRpc, clientParty, (Cgs22CcpsiConfig) config);
            case RS21:
                return new Rs21CcpsiServer<X>(serverRpc, clientParty, (Rs21CcpsiConfig) config);
            default:
    ...
    public static <X> CcpsiClient<X> createClient(Rpc clientRpc, Party serverParty, CcpsiConfig config) {
        ...
        switch (type) {
            case PSTY19:
                return new Psty19CcpsiClient<X>(clientRpc, serverParty, (Psty19CcpsiConfig) config);
            case CGS22:
                return new Cgs22CcpsiClient<X>(clientRpc, serverParty, (Cgs22CcpsiConfig) config);
            case RS21:
                return new Rs21CcpsiClient<X>(clientRpc, serverParty, (Rs21CcpsiConfig) config);
            default:
                throw new IllegalArgumentException("Invalid " + CcpsiType.class.getSimpleName() + ": " + type.name());
        }
    }
