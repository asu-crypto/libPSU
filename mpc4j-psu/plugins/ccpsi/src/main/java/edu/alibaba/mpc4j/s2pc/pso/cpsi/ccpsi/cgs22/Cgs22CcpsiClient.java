...
public class Cgs22CcpsiClient<T> extends AbstractCcpsiClient<T> {
    ...
    public CcpsiClientOutput<T> psi(Set<T> clientElementSet, int serverElementSize) throws MpcAbortException {
        ...
        byte[][] inputArray = new byte[beta][];
        for (int batchIndex = 0; batchIndex < beta; batchIndex++) {
            HashBinEntry<T> item = cuckooHashBin.getHashBinEntry(batchIndex);
            byte[] itemBytes = item.getItemByteArray();
            inputArray[batchIndex] = ByteBuffer.allocate(itemBytes.length + Integer.BYTES)
                .put(itemBytes)
        ...
        byte[][][] truncatedTargetArrays = new byte[targetArrays.length][][];
        for (int i = 0; i < targetArrays.length; i++) {
            byte[][] targetArray = targetArrays[i];
            byte[][] truncatedTargetArray = new byte[targetArray.length][psmByteL];
            for (int j = 0; j < targetArray.length; j++) {
                byte[] target = targetArray[j];
                ...
                truncatedTargetArray[j] = truncatedTarget;
            }
            truncatedTargetArrays[i] = truncatedTargetArray;
        }
        targetArrays = truncatedTargetArrays;
        // P2 inputs y_1^*, ..., y_β^* and outputs z1.
        SquareZ2Vector z1 = pdsmSender.pdsm(psmL, targetArrays);
        // create the table
        ArrayList<T> table = new ArrayList<T>(beta);
        for (int batchIndex = 0; batchIndex < beta; batchIndex++) {
            HashBinEntry<T> item = cuckooHashBin.getHashBinEntry(batchIndex);
            if (item.getHashIndex() == HashBinEntry.DUMMY_ITEM_HASH_INDEX) {
                table.add(null);
            } else {
                table.add(item.getItem());
            }
        }
        CcpsiClientOutput<T> clientOutput = new CcpsiClientOutput<T>(table, z1);
        cuckooHashBin = null;
        stopWatch.stop();
    ...
    private List<byte[]> generateCuckooHashKeyPayload() {
        cuckooHashBin = CuckooHashBinFactory.<T>createEnforceNoStashCuckooHashBin(
        ...
        ArrayList<byte[]> out = new ArrayList<byte[]>(keys.length);
        for (byte[] k : keys) {
            out.add(k);
        }
        return out;
