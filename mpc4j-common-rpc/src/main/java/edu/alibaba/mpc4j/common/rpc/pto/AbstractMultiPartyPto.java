...
public abstract class AbstractMultiPartyPto implements MultiPartyPto {
    ...
    /**
     * RPC counter baselines for {@link #beginPhaseMetric()} / {@link #endPhaseMetric()} (must not call {@link Rpc#reset()}).
     */
    private long phaseMetricBaselineSendBytes;
    /**
     * sub protocols
     */
    ...
    /**
     * Starts a per-phase benchmark interval. Does not reset {@link Rpc} counters so {@code PsuMain} Pto totals stay valid.
     */
    protected void beginPhaseMetric() {
        phaseMetricBaselineSendBytes = rpc.getSendByteLength();
        stopWatch.reset();
        stopWatch.start();
    }

    /**
     * Logs {@code PSU_PHASE_METRIC} (ms and incremental send bytes since {@link #beginPhaseMetric()}).
     */
    protected void endPhaseMetric(String phase, String parallelMode) {
        stopWatch.stop();
        long timeMs = stopWatch.getTime(java.util.concurrent.TimeUnit.MILLISECONDS);
        long sendBytes = rpc.getSendByteLength() - phaseMetricBaselineSendBytes;
        info(
            "PSU_PHASE_METRIC protocol={} party={} phase={} ms={} sendBytes={} parallel={}",
            getPtoDesc().getPtoName(), ownParty().getPartyId(), phase, timeMs, sendBytes, parallelMode
        );
        stopWatch.reset();
    }
