...
public class CwStdIdxPirNativeUtils {
     ...
     */
    public static native byte[] generateReply(byte[] encryptionParams, byte[] galoisKey, byte[] relinKey,
                                              List<byte[]> queryList, byte[][] database,
                                              List<int[]> plaintextIndexCodewords, int numInputCiphers,
     ...
     */
    public static native long[] decryptReply(byte[] encryptionParams, byte[] secretKey, byte[] response);
}
