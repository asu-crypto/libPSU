...
namespace {
Ciphertext evaluate_matches(JNIEnv *env, const EncryptionParameters &parms, SEALContext &context,
                          const vector<Ciphertext> &query_powers, const vector<Plaintext> &plaintexts,
                          uint32_t ps_low_power, const RelinKeys &relin_keys, jbyteArray pk_bytes) {
    if (pk_bytes != nullptr) {
        PublicKey public_key = deserialize_public_key(env, pk_bytes, context);
        return polynomial_evaluation(parms, query_powers, plaintexts, ps_low_power, relin_keys, public_key);
    }
    return polynomial_evaluation(parms, query_powers, plaintexts, ps_low_power, relin_keys);
}
} // namespace
...
jbyteArray JNICALL Java_edu_alibaba_mpc4j_s2pc_upso_upsu_tcl23_Tcl23UpsuNativeUtils_optComputeMatches(
    ...
    vector<Plaintext> plaintexts = deserialize_plaintexts(env, database_coeffs, context);
    Ciphertext f_evaluated = evaluate_matches(env, parms, context, query_powers, plaintexts, ps_low_power, relin_keys,
                                              nullptr);
    Plaintext r = deserialize_plaintext_from_coeff(env, r_coeffs, context);
    if (f_evaluated.is_ntt_form()) {
        evaluator.transform_from_ntt_inplace(f_evaluated);
    }
...
extern "C" [[maybe_unused]] JNIEXPORT
jbyteArray JNICALL Java_edu_alibaba_mpc4j_s2pc_upso_upsu_tcl23_Tcl23UpsuNativeUtils_evalPs(
        JNIEnv *env, jclass, jbyteArray parms_bytes, jbyteArray pk_bytes, jbyteArray relin_keys_bytes,
        jobject database_coeffs, jobject query_list, jint ps_low_power, jlongArray r_coeffs) {
    EncryptionParameters parms = deserialize_encryption_parms(env, parms_bytes);
    SEALContext context(parms);
    RelinKeys relin_keys = deserialize_relin_keys(env, relin_keys_bytes, context);
    Evaluator evaluator(context);
