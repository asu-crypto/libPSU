...
public class Gmr21NetRosnSender extends AbstractNetRosnSender {
    ...
    public RosnSenderOutput rosn(int num, int byteLength) throws MpcAbortException {
        ...
        // One COT batch per level to avoid holding all level outputs in heap (critical at n ≈ 2^20).
        for (int levelIndex = 0; levelIndex < level; levelIndex++) {
            CotSenderOutput cotSenderOutput = cotSender.send(width);
            handleCotSenderOutputsForLevel(cotSenderOutput, levelIndex, totalByteLen);
        }
        stopWatch.stop();
    ...
    private void handleCotSenderOutputsForLevel(CotSenderOutput cotSenderOutput, int levelIndex, int totalByteLen) {
        ...
        if (totalByteLen <= CommonConstants.BLOCK_BYTE_LENGTH) {
            ...
            widthIndexIntStream.forEach(widthIndex -> {
                byte[] otR0 = cotSenderOutput.getR0(widthIndex);
                otR0 = crhf.hash(otR0);
                ...
                byte[] otR1 = cotSenderOutput.getR1(widthIndex);
