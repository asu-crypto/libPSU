...
class Psz18NoStashCuckooHashBin<T> extends AbstractNoStashCuckooHashBin<T> {
    ...
    /**
     * 3 hashes, paper-CSSW25 packing ε = 1.40 (β = ⌈1.4·n⌉). Matches §6 of Chandran et al.
     * (ASIACCS 2025), where the Circuit-PSI Cuckoo table is sized at (1 + 0.4)·n. See
     * {@link CuckooHashBinType#NO_STASH_PSZ18_3_HASH_E04} for the opt-in CSS25 path.
     */
    private static final double H3_E04_EPSILON = 1.40;
    ...
    static double getEpsilon(CuckooHashBinType type, int maxItemSize) {
        ...
        switch (type) {
            ...
            case NO_STASH_PSZ18_3_HASH_E04:
                // 3 hashes, paper-CSSW25 ε = 1.40. Uses the same small-item-size floor as
                // NO_STASH_PSZ18_3_HASH because the underlying hash family is identical; only
                // the packing constant changes (1.27 → 1.40), giving a larger, safer table.
                if (maxItemSize == 1) {
                    return 2.0;
                } else {
                    return Math.max(NoStashCuckooHashBinUtils.getH3SmallItemSizeEpsilon(maxItemSize), H3_E04_EPSILON);
                }
    ...
    static int getMaxItemSize(CuckooHashBinType type, int binNum) {
        ...
        switch (type) {
            ...
            case NO_STASH_PSZ18_3_HASH_E04:
                MathPreconditions.checkGreater("binNum", binNum, (int) Math.floor(getBinNum(type, MAX_SPECIAL_ITEM_SIZE)));
                return (int) Math.floor(binNum / H3_E04_EPSILON);
            case NO_STASH_PSZ18_4_HASH:
                MathPreconditions.checkGreater("binNum", binNum, (int) Math.floor(getBinNum(type, MAX_SPECIAL_ITEM_SIZE)));
